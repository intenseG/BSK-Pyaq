...
class DualNetwork(object):
    ...
    def save_vars(self, sess_, ckpt_path="models/bsk-model-1.ckpt"):
        with tf.get_default_graph().as_default():
    ...
    def save_graph(self, pb_path="bsk-graph-1.pb"):
        tf.train.write_graph(tf.get_default_graph(), './models',
                 pb_path, as_text=True)
